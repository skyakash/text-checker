...
- **Empirical calibration via metrics.** A `rag_retrieval_score{mode}` Prometheus histogram (shipped in a follow-up) observes every chunk's score *before* the floor is applied. Operators tune `RAG_MIN_SCORE` from the actual distribution rather than from guesswork — `histogram_quantile(0.95, …)` per mode tells you exactly where to set it. This was the missing piece that justified picking a single hardcoded default at all: there's now a feedback loop to correct it.
